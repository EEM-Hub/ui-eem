...
- **Q21**: CORRECT — Why is accessibility a shared responsibility?
...
```
{"answer": "c", "explanation": "The semantic HTML accessibility pipeline describes how browsers process HTML into structures that assistive technologies can use. The browser parses HTML into the DOM (Document Object Model), which is then used to construct both the render tree (for visual display) and the accessibility tree (a parallel structure that exposes semantic information to screen readers and other assistive technologies). The accessibility tree strips away purely visual elements and retains meaningful semantic information such as roles, names, states, and relationships derived from the HTML elements. This is why semantic HTML matters for accessibility — elements like <nav>, <main>, <button>, and <h1> carry inherent semantic meaning that gets properly represented in the accessibility tree, whereas generic <div> and <span> elements do not, requiring additional ARIA attributes to convey the same information."}
```


## By Objective

- **Accessibility**: 5/6 (83%)
