...
```
```json
{"answer": "c", "explanation": "The semantic HTML accessibility pipeline describes how browsers process HTML into accessible information for assistive technologies. HTML is parsed into the DOM (Document Object Model), which is then used to create both a render tree for visual display and an accessibility tree. The accessibility tree is a parallel structure that extracts semantic information from the DOM (element roles, names, states, properties) and exposes it to assistive technologies like screen readers through platform accessibility APIs (like MSAA, UIA on Windows, or AX API on macOS). This pipeline is why semantic HTML matters—properly structured markup directly populates the accessibility tree with meaningful information without requiring additional ARIA annotations."}
```
```
